...
export const ScriptMatcherPanel: React.FC = () => {
  ...
  useEffect(() => {
    ...
    setScriptText(c?.scriptText ?? DEFAULT_SCRIPT);
    setUploadInfo(null);
    setUploadError(null);
    setExcerpt(null);
  ...
  const handleFilePicked = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!/\.(pdf|docx)$/i.test(file.name)) {
      setUploadError("PDF 또는 Word(.docx) 파일만 올릴 수 있습니다.");
      return;
    }
    if (file.size > MAX_CLIENT_BYTES) {
      setUploadError(`파일이 너무 큽니다 (${Math.round(file.size / 1024 / 1024)}MB). 최대 10MB.`);
      return;
    }
    setIsUploading(true);
    try {
      ...
      setUploadError(
        err instanceof BackendUnreachableError ? "백엔드 서버에 연결할 수 없습니다." : (err as Error).message
      );
    } finally {
  ...
  const handleRunScriptAnalysis = async () => {
    ...
    setIsAnalyzing(true);
    try {
      const res: ScriptAnalysisResponse = await matchScript(scriptText);
      ...
      saveConversation(threadId, {
        messages: [
          ...base,
          { role: "user", content: `각본 분석 실행 (#${run})` },
          {
            role: "assistant",
            content:
              `[분석 #${run}] ${res.total_scenes_detected}개 씬을 찾아 장소를 매칭했습니다.\n${sceneLines}\n\n` +
      ...
      addThreadLocations(threadId, suggested.map((l) => toThreadLocation(l, `분석 #${run}`)));
    } catch (err) {
      saveConversation(threadId, {
        messages: [
          ...base,
          {
            ...
            content:
              err instanceof BackendUnreachableError
                ? "백엔드 서버에 연결할 수 없어 분석하지 못했습니다."
                : `각본 분석에 실패했습니다. (${(err as Error).message})`,
          },
        ],
      });
    } finally {
  ...
  const handleSendChat = async (textToSend?: string) => {
    ...
    try {
      const res = await sendScoutingChatMessage(updated, thread?.sceneContext, undefined, attached ?? undefined);
      saveConversation(threadId, {
        messages: [
          ...updated,
          {
      ...
      addThreadLocations(threadId, (res.suggested_locations ?? []).map((l) => toThreadLocation(l, "대화 추천")));
    } catch (err) {
        ...
        messages: [
          ...updated,
          {
            ...
            content: msg.includes("503") || msg.includes("GEMINI_API_KEY")
              ? "AI 응답을 받을 수 없습니다 — GEMINI_API_KEY가 설정되지 않았습니다. 키 없이 임의로 추천하지 않습니다."
              : err instanceof BackendUnreachableError
              ? "백엔드 서버에 연결할 수 없습니다."
              : `AI 응답에 실패했습니다. (${msg})`,
          },
    ...
    <div className="flex gap-4 h-[calc(100dvh-13rem)] min-h-[560px]">
                  ...
                  <>
                    ...
                    >
                      ...
                      <div className="text-[11px] text-gray-400 font-medium mt-0.5 truncate">
                        {timeAgo(c.updatedAt)} · 장소 {c.locations?.length ?? 0}
                      </div>
                    </button>
        ...
        <div className="h-[46%] min-h-[240px] flex flex-col bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden">
          <div className="px-5 py-3 flex items-center justify-between flex-wrap gap-2 border-b border-gray-100">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                onClick={() => { setScriptText(DEFAULT_SCRIPT); setUploadInfo(null); setUploadError(null); }}
          ...
          <div className="px-5 pt-3 space-y-2">
            ...
            {uploadInfo && (
              <div className="p-2.5 bg-emerald-50 border border-emerald-200 rounded-xl text-xs font-bold text-emerald-900">
                {uploadInfo.filename} · {uploadInfo.chars.toLocaleString()}자
              </div>
            )}
          </div>
          ...
          <div className="flex-1 min-h-0 px-5 py-3 relative">
            <textarea
              ...
              placeholder="각본 또는 씬 묘사를 입력하세요. 특정 대목을 드래그하면 챗에 첨부할 수 있어요."
            />
            {hasSelection && (
              <button
          ...
          <div className="px-5 pb-4">
            ...
            >
              {isAnalyzing ? <><RefreshCw className="w-4 h-4 animate-spin" />Gemini가 분석 중…</> : <><Sparkles className="w-4 h-4" />분석 및 장소 매칭 시작</>}
            </button>
          </div>
        </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-2">
            {threadLocations.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center text-center text-sm text-gray-400 font-medium gap-1">
                ...
                아직 없습니다. 분석을 돌리거나 AI에게 물어보세요.
              </div>
            ) : (
              threadLocations.map((l) => {
                const scene = threadScenes.find((s) => s.locationId === l.id);
                return (
                  <Link key={l.id} href={`/location/${l.id}?chat=${activeId ?? ""}`}>
                    <div className="group flex items-center gap-3 p-2.5 border border-gray-200 hover:border-indigo-300 hover:bg-indigo-50/40 rounded-xl transition-all mb-2">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="text-xs text-gray-500 font-medium">
                          {l.region} · {l.pricePerHour > 0 ? `₩${l.pricePerHour.toLocaleString()}/h` : "가격 문의"}
                        </div>
                      </div>
      ...
      <div className="w-[36%] min-w-[340px] shrink-0 flex flex-col bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-sm">
        <div className="px-5 py-3 border-b border-gray-200 flex items-center gap-2.5">
          ...
          <div className="min-w-0 flex-1">
            <div className="font-bold text-gray-900 text-sm truncate">{active?.title ?? "새 대화"}</div>
            <div className="text-[11px] text-gray-400 font-medium">
              {messages.length}개 메시지{active && ` · ${timeAgo(active.updatedAt)}`}
            </div>
          </div>
        </div>
              ...
              >
                {msg.error && (
                  <div className="flex items-center gap-1.5 font-bold mb-1">
                    <AlertTriangle className="w-3.5 h-3.5" />응답 실패
                  </div>
                )}
                {msg.content}
              </div>
              ...
              {msg.suggested_locations && msg.suggested_locations.length > 0 && (
                <div className="w-full max-w-[92%] space-y-2 pt-0.5">
                  {Array.from(new Map(msg.suggested_locations.map((l) => [l.id, l])).values()).map((loc) => (
                    <Link key={loc.id} href={`/location/${loc.id}?chat=${activeId ?? ""}`}>
                      <div className="cursor-pointer p-2.5 bg-white hover:bg-gray-50 border border-gray-200 hover:border-indigo-300 rounded-xl flex items-center gap-3 transition-all shadow-sm mb-2">
                        ...
                        <div className="min-w-0">
                          ...
                          <div className="text-[11px] text-gray-500 mt-0.5">
                            {loc.region} · {loc.price_per_hour > 0 ? `₩${loc.price_per_hour.toLocaleString()}/h` : "가격 문의"}
                          </div>
                        </div>
                      </div>
                    </Link>
        ...
        <div className="px-4 py-2.5 bg-white border-t border-gray-200 flex items-center gap-2 overflow-x-auto scrollbar-none">
          {[
            { label: "자연/숲", icon: Trees, query: "좀 더 자연이나 숲 느낌이 나는 곳으로 추천해줘" },
            { label: "10만원 이하", icon: DollarSign, query: "시간당 10만원 이하 가성비 좋은 곳으로 찾아줘" },
            { label: "전통 한옥", icon: Landmark, query: "전통 한옥이나 고택 느낌 장소도 추천해줘" },
            { label: "주차 넓은 곳", icon: Truck, query: "대형 탑차 주차가 편한 곳은?" },
          ].map((chip, idx) => (
            <button
              key={idx}
        ...
        <div className="p-3.5 bg-white border-t border-gray-200 flex items-center gap-2.5">
          <input
            ...
            placeholder={excerpt ? "첨부한 대목에 대해 물어보세요…" : "AI에게 조건을 말해보세요…"}
